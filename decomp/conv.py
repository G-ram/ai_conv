...
class ConvSepDHV(Conv):
    def __init__(self, shape, name=None, init1=None, init2=None, 
            init3=None, stride=[1, 1, 1, 1], padding='SAME'):
        ...
        dtype = tf.float16 if FLAGS.use_fp16 else tf.float32
        if init1 == None:
            self.init1 = tf.truncated_normal_initializer(stddev=5e-2, dtype=dtype)
        if init2 == None:
            self.init2 = tf.truncated_normal_initializer(stddev=5e-2, dtype=dtype)
        if init3 == None:
            self.init3 = tf.truncated_normal_initializer(stddev=5e-2, dtype=dtype)
    ...
    def set_input(self, input):
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            kernel1 = _variable_with_weight_decay('weights_d', conv1_shape, self.init1)
            # kernel shape: rows, 1, filters, 1
            kernel2 = _variable_with_weight_decay('weights2_h', conv2_shape, self.init2)
            # kernel shape: 1, cols, filters, 1
            kernel3 = _variable_with_weight_decay('weights3_v', conv3_shape, self.init3)
...
class ConvSepHV(Conv):
    ...
    def set_input(self, input):
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            kernel1 = _variable_with_weight_decay('weights_h', conv1_shape, self.init1)
            kernel2 = _variable_with_weight_decay('weights_v', 
                conv2_shape + [filters], self.init2)
...
class ConvSep2D(Conv):
    ...
    def set_input(input):
        ...
        with tf.variable_scope(self.name) as scope:
            # kernel shape: rows, cols, channels, filters
            kernel_depth = _variable_with_weight_decay('weights_d', self.shape, self.init1)
            kernel_point = _variable_with_weight_decay('weights_p', self.shape, self.init1)
...
class Conv3D(Conv):
    ...
    def set_input(self, input):
        ...
        with tf.variable_scope(self.name) as scope:
            # kernel shape: rows, cols, channels, filters
            kernel = _variable_with_weight_decay('weights', self.shape, self.init1)
